...
bool InGame::init()  
{
	...
	SimpleAudioEngine::getInstance()->preloadBackgroundMusic("cell.mp3");
	SimpleAudioEngine::getInstance()->playBackgroundMusic("cell.mp3",true);
	...
	sprite3 = Sprite::create("BackScreen/cell2.jpg");
...
void InGame::initScore() {
	auto visibleSize = Director::getInstance()->getVisibleSize();
	auto labelScore = Label::createWithSystemFont("Eat Point : 0", "fonts/a몬스터.ttf", 35);
	...
	auto labelSize = Label::createWithSystemFont("Dust Size Level : 1", "fonts/a몬스터.ttf", 35);
	...
	auto labelGG = Label::createWithSystemFont("Sperm\n\nAmeba\n\nRedCell\n\nWhiteCell\n\nPlantCell\n\nAnimalCell\n\nColdCell\n\nNanja\n\nTcell", "fonts/a몬스터.ttf", 25);
	labelGG->setAnchorPoint(Point(0, 1));
	labelGG->setPosition(Point(visibleSize.width - 155, visibleSize.height - 70));
	labelGG->setColor(Color3B::YELLOW);
	labelGG->setTag(TAG_LABEL_GG);
	this->addChild(labelGG, 100);
}

void InGame::addScore(int add, int add2) {
	ep += add;
...
void InGame::onTouchesMoved(const std::vector<cocos2d::Touch*>& touches, cocos2d::Event * unused_event)
{
...
void InGame::onTouchesEnded(const std::vector<cocos2d::Touch*>& touches, cocos2d::Event * unused_event)
{
}
...
void InGame::update(float dt) 
{
	...
	if (sprite->getPositionX() >= 1260 && sprite3->getPositionX()>-8192+1920) {
		float x = sprite->getPositionX()-1260;
		sprite3->setPositionX(sprite3->getPositionX() - x);
		sprite->setPositionX(1260);
		for (std::vector<Enemy>::iterator iter = enemyVec.begin(); iter != enemyVec.end(); iter++) {
	...
	if (sprite->getPositionY() >= 690 && sprite3->getPositionY() > -4608+1080) {
	...
	if (sprite->getPositionY() <= 480 && sprite3->getPositionY() < 0) {
		...
		if (g < 200)
			schedule(schedule_selector(InGame::Eadd), 0.2);
		if (g == 200)
			unschedule(schedule_selector(InGame::Eadd));
	...
	for (std::vector<Enemy>::iterator iter = enemyVec.begin(); iter != enemyVec.end();) {
		if ((*iter).spriteE != NULL) {
			...
			if (playerRect.intersectsRect(enemyRect)) {
				...
				if ((*iter).es > ps) {
					...
					(*iter).spriteE = NULL;
					iter = enemyVec.erase(iter);
					if (ep == 5 || ep == 10 || ep == 15 || ep == 20 || ep == 25 || ep == 30 || ep == 35 || ep == 40 || ep == 45) {
						addScore(0, 1);
						pSize = pSize + 0.03;
	...
	//	Director::getInstance()->getEventDispatcher()->removeEventListener(listener);
		Director::getInstance()->replaceScene(InGame2::createScene());
	}
}
