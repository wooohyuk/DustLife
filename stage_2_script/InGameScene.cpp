...
void InGame::initScore() {
	...
	auto labelGG = Label::createWithSystemFont("SeaRabbit\n\nShrimp\n\nHaePari\n\nJungAuri\n\nLuckyFish\n\nBlobFish\n\nBlueFish\n\nTurtle\n\nDogfish\n\nShark", "fonts/a몬스터.ttf", 25);
...
void InGame::update(float dt) 
{
	...
	if (sprite->getPositionY() <= 480 && sprite3->getPositionY() < 0) {
		...
		if (g < 200)
			schedule(schedule_selector(InGame::Eadd), 0.6);
		if (g == 200)
		...
		if ((*iter).spriteE != NULL) {
			...
			if (playerRect.intersectsRect(enemyRect)) {
				...
				if ((*iter).es > ps) {
					...
					(*iter).spriteE = NULL;
					iter = enemyVec.erase(iter);
					if (ep == 55 || ep == 60 || ep == 75 || ep == 80 || ep ==85 || ep == 90 || ep == 95 || ep == 100 || ep == 105) {
						addScore(0, 1);
						pSize = pSize + 0.03;
